...
for filename in meter_readings_filenames:
    ...
    output_path = os.path.join(meter_readings_directory, f"{base_name}hourly_{date_part}.parquet")
    
    # Process file
    try:
        lazy_hourly_df = (
            pl.scan_parquet(input_path)
            .sort("date")  # ← Critical: Sort by time first
            .group_by_dynamic(
                index_column="date",
                every="1h",
                group_by=["ca_id", "city"]
