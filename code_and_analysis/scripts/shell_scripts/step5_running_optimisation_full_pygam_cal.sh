...
#PBS -N step5_running_optimisation_full_pygam_cal
...
SCRIPT="$PBS_O_WORKDIR/step5_running_optimisation_full_pygam_cal.py"
if [[ ! -f "$SCRIPT" ]]; then
...
mpiexec --report-bindings $MAP $HOST -n "$NP" \
  ...
  python -u "$SCRIPT" --solver greedy --policy p2
